...
- **Changing serializers can strand data**: No fallback parser runs by default. An explicit `fallbackParser` runs only when the configured parser throws; use a new prefix and plan [format migration](/guides/destinations#custom-serialization).
