...
Storage access, serialization-on-write, updater, and thrown schema errors propagate. Async schemas
throw. Quota failures also propagate unless the instance's opt-in
[quota recovery](/reference/storage#recovering-from-quota-failures) succeeds. Configure
`onQuotaExceeded: 'clear-expired'` on `createStorage()` to clear expired entries and retry once;
the hook setter's updater runs only once.
